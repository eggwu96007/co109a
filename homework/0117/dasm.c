//從最後函式一個看
/* sprintf 把印出來的東西丟到head sprintf(head, "%s=", dTable[d]); */
#include <string.h>
#include <stdint.h>
#include <stdio.h>
#include <stddef.h>
#include <assert.h> /*除錯用*/ 
#include <stdlib.h>
...
char *dTable[] = {"", "M", "D",  "MD",  "A",   "AM",  "AD",  "AMD"};//D表格
...
int main(int argc, char *argv[]) {
  ...
  FILE *binFile = fopen(binFileName, "rb");//read binary
  imTop = fread(im, sizeof(uint16_t), 32768, binFile);//bin read to im
  fclose(binFile);
  disasm(im, imTop); //返組譯
  return 0;
}
