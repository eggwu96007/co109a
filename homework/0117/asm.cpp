...
void pass1(char *inFile) { /*讀ASM黨*/
  ...
  while (fgets(line, sizeof(line), fp)) {
    ...
    if (strlen(code)==0) continue;//空行繼續
    printf("%02d:%s\n", address, code);//02 指定兩碼
    if (code[0] == '(') {
      char label[100];
      sscanf(code, "(%[^)])", label);//類似正規表達式
      string lab(label);
      if (symMap.find(lab) == symMap.end())
         symMap[lab] = address; // 記住符號位址，給 pass2 編碼時使用
      else
         printf("error: %s already defined before !", label);//找到2次loop就錯誤
...
void code2binary(char *code, char *binary) {
  if (code[0]=='@') { // A 指令： ＠number || @symbol
    int address;
    int match = sscanf(code, "@%d", &address); //scanf掃到幾個東西
    if (match == 1)
    ...
    if (strchr(code, '=') != NULL) { // d=comp(c欄位)
      sscanf(code, "%[^=]=%s", d, comp);//開始查表
